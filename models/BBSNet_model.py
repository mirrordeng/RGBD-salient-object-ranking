...
from models.dsam import Adaptive_DSAM, Adaptive_DSAMsub1, Adaptive_DSAMsub2, Adaptive_DSAMsub3
...
class BBSNet(nn.Module):
    def __init__(self, channel=32):
        ...
        self.depth_att0_1 = Adaptive_DSAMsub1(32)
        self.depth_att0_2 = Adaptive_DSAMsub2(32)
        self.depth_att0_3 = Adaptive_DSAMsub3(32)
        self.depth_att0_4 = Adaptive_DSAMsub3(32)
        self.depth_att0 = Adaptive_DSAM(96)
        self.depth_att1 = Adaptive_DSAM(64)
        self.depth_att2 = Adaptive_DSAM(256)
        self.depth_att3 = Adaptive_DSAM(512)
        self.depth_att4 = Adaptive_DSAM(1024)
        self.depth_att5 = Adaptive_DSAM(2048)

        self.out_final1 = nn.Conv2d(4, 32, kernel_size=1, stride=1, bias=True)
        self.BN_fianl = nn.BatchNorm2d(32, eps=1e-05, momentum=0.1, affine=True)
        self.relu_final = nn.ReLU(inplace=True)
        self.out_final2 = nn.Conv2d(32, 4, kernel_size=1, stride=1, bias=True)
        ...
        self.agant1 = self._make_agant_layer(32, 32*2)
        ...
        self.pool_b = nn.Sequential(
            nn.MaxPool2d(2, stride=2, ceil_mode=True),
            nn.MaxPool2d(2, stride=2, ceil_mode=True),
        )
        self.pool_0 = nn.MaxPool2d(2, stride=2, ceil_mode=True)

        if self.training:
            self.initialize_weights()

    def forward(self, x, x_depth, bins_mask, gumbel=True):
        b = bins_mask
        ...
        x1_2 = self.rfb1_2(x1)
        x5_2 = self.rfb5_2(x5)
        x5_3 = self.upsample2(x5_2)
        # print("return", x0_2.shape, x1_2.shape, x5_2.shape)
        # y = self.agg2(x5_2, x1_2, x0_2) #*4
        # print(y.shape)

        # y_0 = self.depth_att0(y, b1)

        #PTM module
        y_1 = self.depth_att0_1(x0_2, b1)
        y_1 = x0_2 +y_1
        y_1 =self.agant1(y_1)
        y_1 = self.deconv1(y_1)
        y_1 = self.agant2(y_1)
        y_1 = self.deconv2(y_1)
        y_1 = self.out2_conv(y_1)

        y_2 = self.depth_att0_2(x1_2, b1)
        y_2 = x1_2 + y_2
        y_2 = self.agant1(y_2)
        y_2 = self.deconv1(y_2)
        y_2 = self.agant2(y_2)
        y_2 = self.deconv2(y_2)
        y_2 = self.out2_conv(y_2)

        y_3 = self.depth_att0_3(x5_3, b1)
        y_3 = x5_3 + y_3
        y_3 = self.agant1(y_3)
        y_3 = self.deconv1(y_3)
        y_3 = self.agant2(y_3)
        y_3 = self.deconv2(y_3)
        y_3 = self.out2_conv(y_3)

        # y_4 = self.depth_att0_3(x3_2, b1)
        # y_4 = x3_2+ y_4
        # y_4 = self.agant1(y_4)
        # y_4 = self.deconv1(y_4)
        # y_4 = self.agant2(y_4)
        # y_4 = self.deconv2(y_4)
        # y_4 = self.out2_conv(y_4)
        y_4 = self.upsample(attention_map)

        y_4_0 = y_4.sigmoid()
        y_1_0 = y_1.sigmoid()
        y_2_0 = y_2.sigmoid()
        y_3_0 = y_3.sigmoid()
        y_1_0 = (y_1_0 - y_1_0.min()) / (y_1_0.max() - y_1_0.min() + 1e-8)
        y_2_0 = (y_2_0 - y_2_0.min()) / (y_2_0.max() - y_2_0.min() + 1e-8)
        y_3_0 = (y_3_0 - y_3_0.min()) / (y_3_0.max() - y_3_0.min() + 1e-8)
        y_4_0 = (y_4_0 - y_4_0.min()) / (y_4_0.max() - y_4_0.min() + 1e-8)
        y_2_0 = y_2_0 - y_1_0
        y_3_0 = y_3_0 - y_2_0 - y_1_0
        y_4_0 = y_4 - y_3_0 - y_2_0 - y_1_0
        final_y = torch.cat((y_1_0, y_2_0, y_3_0, y_4_0), 1)

        final_y = self.out_final1(final_y)
        final_y = self.BN_fianl(final_y)
        final_y = self.relu_final(final_y)
        final_y = self.out_final2(final_y)
        return y_4 ,y_1, y_2, y_3,final_y

        final_y = self.out_final1(final_y)
        final_y = self.BN_fianl(final_y)
        final_y = self.relu_final(final_y)
        final_y = self.out_final2(final_y)
        return self.upsample(attention_map),y_1, y_2, y_3, final_y
